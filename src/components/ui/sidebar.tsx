...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "bg-background relative flex w-full flex-1 flex-col overflow-x-visible md:overflow-x-hidden xl:overflow-x-visible",
...
function SidebarFooter({
  ...
  const normalizeTo = (url: string) => (url.startsWith("#") ? `/${url}` : url);
        ...
        <SidebarMenu>
          {items.map((item) => (
            <SidebarMenuItem key={item.title}>
              <SidebarMenuButton asChild onClick={closeSidebar} size="sm">
