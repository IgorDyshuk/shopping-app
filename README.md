...
- `src/stores/use-cart.ts` — корзина с количеством и размером позиции, persist в localStorage, интеграция с карточками/страницей товара и мини-просмотром.
- `src/stores/use-favorites.ts` — избранное (persist), общий источник для карточек, корзины и страницы товара.
...
- `src/pages/Product.tsx` — страница товара: галерея, рейтинг, выбор размера (синхронизирован с корзиной, подхватывает выбранный размер при возврате), CTA, аккордеоны, описание/характеристики, подборки по категории и недавно просмотренные, липкое меню секций.
